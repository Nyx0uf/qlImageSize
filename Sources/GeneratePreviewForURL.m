...
OSStatus GeneratePreviewForURL(void *thisInterface, QLPreviewRequestRef preview, CFURLRef url, CFStringRef contentTypeUTI, CFDictionaryRef options)
{
	...
	if (st.st_size > 1048576) // More than 1Mb
		fmtSize = [NSString stringWithFormat:@"%.2fMb", (float)((float)st.st_size / 1048576.0f)];
	else if (st.st_size < 1048576 && st.st_size > 1024) // 1Kb - 1Mb
		fmtSize = [NSString stringWithFormat:@"%.2fKb", (float)((float)st.st_size / 1024.0f)];
	else // Less than 1Kb
		fmtSize = [NSString stringWithFormat:@"%ldb", st.st_size];

	CFStringRef keys[1] = {kQLPreviewPropertyDisplayNameKey};
	CFStringRef values[1] = {CFStringCreateWithFormat(kCFAllocatorDefault, NULL, CFSTR("%@ (%dx%d - %@)"), filename, width, height, fmtSize)};
